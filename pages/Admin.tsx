...
interface AdminProps {
  data: PortfolioData;
  onUpdateSettings: (settings: PortfolioData['settings']) => Promise<void> | void;
  onAddProject: (project: Project) => Promise<void> | void;
  onDeleteProject: (id: string) => Promise<void> | void;
  onMarkLeadRead: (id: string) => Promise<void> | void;
  onLogout?: () => void;
}

const Admin: React.FC<AdminProps> = ({ data, onUpdateSettings, onAddProject, onDeleteProject, onMarkLeadRead, onLogout }) => {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-20 gap-12">
          ...
          <div className="flex items-center gap-4">
            ...
            {onLogout && (
              <button
                onClick={onLogout}
                className="px-5 py-3 rounded-2xl text-xs font-bold uppercase tracking-widest text-slate-400 border border-slate-800 hover:text-red-400 hover:border-red-500/40 transition-all whitespace-nowrap"
              >
                Log Out
              </button>
            )}
            ...
            <div className="space-y-6">
              ...
              {data.projects.map(project => (
                <div key={project.id} className="group flex items-center justify-between p-6 bg-slate-950/50 border border-slate-800 rounded-[2rem] hover:border-blue-500/50 transition-all duration-500">
                  ...
                  <button 
                    onClick={() => Promise.resolve(onDeleteProject(project.id)).catch((err: any) => alert(`Failed to delete: ${err.message || err}`))}
                ...
                {data.leads.map(lead => (
                  <div key={lead.id} className={`p-10 rounded-[2.5rem] border transition-all duration-500 ${lead.read ? 'bg-slate-950/50 border-slate-800' : 'bg-blue-500/5 border-blue-500/30 ring-1 ring-blue-500/20 shadow-2xl shadow-blue-900/10'}`}>
                    <div className="flex flex-col md:flex-row justify-between items-start mb-8 gap-4">
                      ...
                      {!lead.read && (
                        <button 
                          onClick={() => Promise.resolve(onMarkLeadRead(lead.id)).catch((err: any) => alert(`Failed to update: ${err.message || err}`))}
